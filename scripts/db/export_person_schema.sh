...
SCHEMA_PATH="$ROOT_DIR/modules/person/infrastructure/sqlc/schema.sql"
TABLES=(
  persons
)
...
if [[ "${SKIP_MIGRATE:-0}" != "1" ]]; then
  ...
  (cd "$ROOT_DIR" && PERSON_MIGRATIONS=1 make db migrate up)
fi
...
echo "Exported Person schema to $SCHEMA_PATH"
