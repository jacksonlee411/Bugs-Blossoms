...
CREATE TABLE org_job_families (
	...
	CREATE UNIQUE INDEX org_job_profile_job_families_primary_unique
	    ON org_job_profile_job_families (tenant_id, job_profile_id)
	    WHERE is_primary = TRUE;

	CREATE INDEX org_job_profile_job_families_tenant_family_profile_idx
	...
	BEGIN
	    ...
	    SELECT EXISTS (
	        ...
	        WHERE p.tenant_id = t_id AND p.id = p_id) INTO parent_exists;
	    ...
	    SELECT
	        ...
	        COALESCE(SUM(CASE WHEN is_primary THEN 1 ELSE 0 END), 0) INTO sum_pct,
	        primary_count
	    FROM
	...
	CREATE UNIQUE INDEX org_position_slice_job_families_primary_unique
	    ON org_position_slice_job_families (tenant_id, position_slice_id)
	    WHERE is_primary = TRUE;

	CREATE INDEX org_position_slice_job_families_tenant_family_slice_idx
	...
	BEGIN
	    ...
	    SELECT EXISTS (
	        ...
	        WHERE s.tenant_id = t_id AND s.id = s_id) INTO parent_exists;
