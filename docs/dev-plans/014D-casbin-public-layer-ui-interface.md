...
6. [ ] 文档与示例 —— 更新 README/CONTRIBUTING/AGENTS，补充公共层接入流程、Unauthorized 数据字段示例、403 JSON 示例（同上）、调用 `/core/api/authz/requests|debug` 示例链接；新增统一 Unauthorized 组件迁移说明：以 `modules/core/presentation/templates/components/unauthorized.templ` 为唯一实现，替换 HRM/Logging/模块内的临时占位并列出替换清单与回归项；完成后在 dev-records 记录命令与结论。
...
- Unauthorized 体验一致：403 响应与模板占位携带 MissingPolicies、SuggestDiff、subject/domain/object/action、authz requests/debug 参数，HX/REST 同步；未授权仓储不被调用；统一 Unauthorized 组件已替换临时占位，替换清单与回归结果记录完毕。
