...
- 015C 提供“直接维护生效策略 + 调试”API（`/core/api/authz/policies/apply`、`/core/api/authz/debug`），需要公共层保证字段、入口、导航可见性一致且无 legacy fallback。
...
2. UI 契约完善：Unauthorized 占位与页面 props 暴露 MissingPolicies、subject/domain/object/action、debug 参数（`/core/api/authz/debug`），并对齐 015C 的 apply 入口（`/core/api/authz/policies/apply`）；HX/REST 403 payload 一致。
...
4. [x] Unauthorized 契约对齐 —— 为三模块的 Unauthorized props 定义统一结构（MissingPolicies、subject/domain/object/action、/core/api/authz/debug 参数），提供字段示例；统一 HX/REST 403 payload 字段名/结构，示例：`{\"error\":\"forbidden\",\"message\":\"Forbidden: <obj> <act>.\",\"object\":\"<obj>\",\"action\":\"<act>\",\"subject\":\"<subj>\",\"domain\":\"<tenant>\",\"missing_policies\":[...],\"debug_url\":\"/core/api/authz/debug\"}`；更新 controller 403 响应与模板占位，确保 HX-Retarget 兼容；必要时在 pkg/templates/components 或模块 templates/components 复用。
5. [ ] 导航/Quick Links/Spotlight 统一过滤 —— 确保导航/Quick Links/Spotlight 数据源使用 `.RequireAuthz` 或 `AuthzObject/AuthzAction`，移除 legacy `.RequirePermissions`；用有/无目标 capability 账号各验证一次 Core/HRM/Logging sidebar/spotlight 可见性（含 `logging.logs:view`），验证结果回填 docs/dev-records/DEV-PLAN-014-CASBIN-ROLLING.md。
6. [ ] 文档与示例 —— 更新 README/CONTRIBUTING/AGENTS，补充公共层接入流程、Unauthorized 数据字段示例、403 JSON 示例（同上）、调用 debug/apply 示例链接；新增统一 Unauthorized 组件迁移说明：以 `modules/core/presentation/templates/components/unauthorized.templ` 为唯一实现，替换 HRM/Logging/模块内的临时占位并列出替换清单与回归项；完成后在 dev-records 记录命令与结论。
7. [ ] 验证与记录 —— 运行 `go test ./pkg/authz/...` 及受影响模块包测试（含 `go test ./modules/logging/...`）；若改模板/locale，执行 `templ generate && make css`、`make check tr` 并确认 Unauthorized/PolicyInspector 文案覆盖 en/ru/uz/zh，记录到 docs/dev-records/DEV-PLAN-012-CASBIN-POC.md 与 014 rolling，对应责任人/日期需注明。
...
- Unauthorized 体验一致：403 响应与模板占位携带 MissingPolicies、subject/domain/object/action、debug_url/base_revision/request_id 等字段，HX/REST 同步；未授权仓储不被调用；统一 Unauthorized 组件已替换临时占位，替换清单与回归结果记录完毕。
- 三模块均通过相关测试；如修改模板/locale，`templ generate && make css`、`make check tr` 后 `git status --short` 干净；覆盖 Core/HRM/Logging（含 `go test ./modules/logging/...`），并在 dev-records/README/CONTRIBUTING/AGENTS 记录测试命令、责任人与多语言覆盖（en/ru/uz/zh）。
