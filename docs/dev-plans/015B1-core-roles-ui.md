...
3. [X] 暂存与提交链路 —— `POST/DELETE /core/api/authz/policies/stage`（按 subject/domain 隔离，最多 50 条）；HTMX 返回最新 partial，高亮暂存状态（淡绿新增、淡红删除线+“暂存”标签）；`POST /core/api/authz/requests` 读取暂存为 `suggested_diff` 并清空，切换 subject/domain 自动 reset，刷新不丢暂存。
