...
3. [X] 暂存与提交链路 —— `POST/DELETE /core/api/authz/policies/stage`（按 subject/domain 隔离，最多 50 条）；HTMX 返回最新 partial，高亮暂存状态（淡绿新增、淡红删除线+“暂存”标签）；`POST /core/api/authz/policies/apply` 可在 payload 缺省时从暂存区读取变更并清空，切换 subject/domain 自动 reset，刷新不丢暂存；新增集成测试验证成功链路。
...
4. [X] 立即生效串联（阶段 4）—— Apply 按钮可在 payload 缺省时从暂存区读取变更调用 `/core/api/authz/policies/apply` 并清空暂存；HTMX/权限/校验失败返回 toast 提示，补充空暂存与无权限链路集成测试。
...
- [X] 视图提示优化：提交/删除时在矩阵局部展示计数/提示，避免只靠 toast。
