...
- 事件与 outbox：所有写操作成功后发布 `OrgChanged` / `OrgAssignmentChanged` 事件（对齐 022 契约，含 event_id/event_version/sequence/effective_window/assignment_type），outbox 与事务一致，幂等键生成规则明确（event_id 来源、sequence 生成策略）。
- API/模板：REST/HTMX 控制器接受 `effective_date` 查询点，返回显式值与继承解析值占位；基础树/表单 templ 页面提供节点/Position/Assignment 创建编辑，遵守 `templ generate && make css` 生成流程。路由与权限对齐 020/026：节点/Position/Assignment 的 GET 需 `Org.Read`，写入需 `Org.Write`/`Org.Assign`/`Org.Admin` 匹配。
...
5. [ ] 性能基线：复用 020/027 基准或新增 bench，验证 1k 节点树查询 <200ms（PG17 标准环境，固定数据集）。验收：基准命令、数据集与结果记录在 `docs/dev-records/DEV-PLAN-024-READINESS.md`，若不达标提供优化/降级方案。
6. [ ] templ 页面与文档：提供基础树/表单 templ 页面（节点/Position/Assignment 创建编辑），更新模块文档/README，执行 `templ generate && make css`。验收：生成后 `git status --short` 干净。
7. [ ] Readiness：执行 `make check lint`、`go test ./modules/org/...`（或影响路径），必要时 `make check tr` 如有文案变更；将命令/耗时/结果记录到 `docs/dev-records/DEV-PLAN-024-READINESS.md`。
