...
**对齐更新**：
- 2025-12-29：职位体系已由 `DEV-PLAN-072` 对标 Workday 重新收口（移除 `Job Role`/allowed-levels，`Job Level` 改为 tenant-global，Position 写入口仅 `job_profile_id` + 可选 `job_level_code`）。本文保留为历史记录，不再作为实现 SSOT。
- 2025-12-30：`Job Catalog` 二级菜单中文名修正为“职位分类”（`Profiles` 页签为“职位模板”；见 `DEV-PLAN-072A`）。
