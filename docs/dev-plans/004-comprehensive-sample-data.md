...
## 背景
当前只有核心用户/权限数据，其他模块通过 TestKit 或 UI 操作均无示例记录，无法直观演示 ERP 功能。为提升展示与测试效率，需要构建一套“全模块”种子流程，让 HRM、Logging、Website、BI‑Chat 等关键模块在启动后即时拥有不少于 100 条业务数据（表单、对话、日志、员工等）。

## 目标
- 在单次执行脚本（或 `/__test__/seed` 场景）后，即可在所有启用模块看到可演示的列表。
- 至少生成 100 条具有业务意义的数据：如 20+ 员工、若干岗位/组织、若干 AI‑Chat/BI‑Chat 对话、若干日志等，覆盖不同状态/类型。
...
1. **数据域梳理与规模规划**  
   - 列出所有启用模块及核心实体：用户、角色、员工/岗位、对话/消息、网站内容、日志等。  
   - 为每类实体设定数量目标与字段覆盖（不同状态、货币、日期范围），汇总成数据矩阵，确保总体 >100 条记录。

2. **TestKit 场景设计与 JSON 规范**  
   - 扩展 `modules/testkit/services/test_data_service.go` 场景描述，加入 hrm/logging/website/bichat 等字段示例；定义引用方式（`@employees.alice` 等）。  
   - 追加一个“full_demo”或“enterprise”场景，与 `/__test__/seed` API 联动，可清空再填充。

3. **PopulateService 功能补全**  
   - 在 `modules/testkit/services/populate_service.go` 按当前启用模块补齐 TODO（例如 HRM 员工/岗位、BiChat 对话、Website 相关数据、日志等），调用各模块仓储/服务并处理引用。  
