...
- 建议引入开关控制读路径切换（阶段 C）：例如 `VALID_TIME_DATE_MODE=legacy|shadow|enforce`，以支持无停机回滚与灰度验证。
- 建议增加观测：
  - 统计仍使用 RFC3339 作为 Valid Time 输入的请求数（兼容期内用于发现调用方）。
  - 在 `shadow` 模式下对比 date 口径与 legacy 口径的查询结果差异（仅用于阶段 C 的一致性验证）。
