...
- 草稿/请求页面（Requests Center）：015C 后已移除（不再需要 requests 列表/详情页）
...
### 1) 工作流术语（统一口径）
- Stage（暂存）：页面内“未提交”的变更集合（可清空/撤销/导出）；用于生成 Apply 变更，不代表已写入生效策略。
- Apply（立即生效）：通过 `POST /core/api/authz/policies/apply` 将变更直接写入 `policy.csv`/`.rev` 并触发 reload；响应返回 `base_revision/revision/added/removed`。
- 生效（Effective）：`policy.csv`/`.rev` 已更新且服务端完成 reload；UI 统一将其表达为“已生效”，并提供可追踪信息（revision、request_id 关联日志）。
...
- 引入“Authz 工作区（Workspace）”统一容器（可作为 Core 级组件）：
  ...
  - 功能：展示当前 subject/domain、暂存数量、变更摘要（+N/-M、涉及资源/动作）、Apply 按钮、清空按钮、导出 diff（建议支持颜色编码的可视化 Diff）、最近一次 apply 结果（revision/added/removed）。
...
- P0：015C 后不再补齐 requests 详情页；策略变更统一走 stage/apply，排障统一走 debug/request_id 串联日志。
...
2. [x] 015C 之后不再维护 requests 详情页；以 roles/users 的 policies 页面作为唯一在线入口。
...
- HTMX/REST/非 HTMX 的 403/错误反馈与契约一致；通过 `request_id` + `debug_url` 可追踪排障。
...
2. **页面与控制器**：
   - 015C 之后不再保留 requests 详情/列表页面与对应控制器；管理员策略变更统一通过 `POST /core/api/authz/policies/apply` 直接生效。
3. **导航入口**：
   - 移除 requests center 的侧边栏/Spotlight/Quick Links 入口；保留 roles/users 的 policies 页面作为唯一在线入口。
4. **体验与一致性**：
...
- **REST(JSON)**：对任意受保护 endpoint，带 `Accept: application/json` 时返回 `403` 且 `Content-Type: application/json`；payload 至少包含 `error/message/object/action/subject/domain/missing_policies/debug_url/base_revision/request_id`（其中 `base_revision/request_id` 允许为空但推荐透出）。
- **HTMX(HTML)**：对页面类 endpoint，带 `Hx-Request: true` 且非 JSON Accept 时返回 `403`，并设置 `Hx-Retarget: body`、`Hx-Reswap: innerHTML`；body 渲染 `components/authorization/unauthorized.templ`（至少包含 `data-authz-container`、`data-debug-url`、`data-base-revision`）。
...
- **自动化覆盖建议**：至少覆盖 Core/HRM/Logging 三模块的 `ensure*Authz`（JSON/HTMX/HTML fallback）与关键 API（例如 `/core/api/authz/policies/apply` 的 `AUTHZ_BASE_REVISION_MISMATCH`、`Hx-Trigger: showErrorToast/notify`）。
