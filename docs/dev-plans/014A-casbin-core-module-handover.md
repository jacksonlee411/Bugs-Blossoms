...
- **服务层 & API 授权加固**
  ...
  - [ ] 检查后台作业/事件订阅、定时任务、Websocket 推送在无 HTTP context 下的授权策略，明确采用 system subject 或隔离队列，并补充决策记录。
  - [ ] 盘点 Core 模块所有 REST/GraphQL API（除 `/core/api/authz/*` 外）在加入 middleware 后是否仍需细粒度守卫，必要时补充 `ensureAuthz`/service guard。
- **UI/模板消费 ViewState**
...
- **灰度与文档**
  ...
  - [ ] 启动 `docs/dev-records/DEV-PLAN-014-CASBIN-ROLLING.md`，登记 Core 模块的 shadow/enforce 启停命令、观测、回滚演练，并附上 diff 结论。
  - [ ] 汇总上述改造经验，形成可供 HRM/Logging 复用的 check list / best practice，明确“什么时候切 014A → 014 主计划 M1”的判定标准。
  - [ ] 明确与 HRM/Logging 的依赖：在 M3 完成前不得移除 `user.Can` fallback；完成后通知主计划可复用组件/脚手架。
