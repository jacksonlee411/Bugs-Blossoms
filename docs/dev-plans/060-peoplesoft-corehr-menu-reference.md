...
本文档记录 PeopleSoft Core HR（核心人力资源）的完整功能菜单结构，涵盖 21 个功能菜单入口（假期与请假并入“8. 时间与考勤”；薪资计算按模块边界口径并入“6. 薪酬管理”；职位主数据沉淀为“2.1 职位体系”，员工任职数据独立为“5. 员工任职记录”），包括：
...
### 说明
围绕劳动合同全生命周期管理，提供模板、签署、续签、派遣外包、合规审计的集中治理，与人员管理模块通过 `person_id`、`employment_id` 进行关联。合同签署/续签/变更/解除/终止等流程在本模块治理；如需同步触发入转调离/状态生效由 **"4. 人事事件"** 承担，纠纷/处分等协同 **"18. 员工关系"**，合规审计协同 **"16. 合规管理"**。模块需要与薪资等场景联动，确保符合《劳动合同法》《劳动派遣暂行规定》等监管要求，并支持提醒与审计追踪。
