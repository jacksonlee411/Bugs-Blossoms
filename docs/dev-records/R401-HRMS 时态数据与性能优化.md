...
1. **有效时间 + 事务审计（Workday 式 Effective Dating）**：业务主表以生效日期（Effective Date）驱动版本，事务时间（Transaction Time）通常通过审计日志/事件流记录，而非在主表直接堆叠双时间维；若需要严格双时态（Valid+Tx）再在独立历史/审计表承载。主表保持单时态便于约束与性能，审计层满足合规回放 1。  
...
4. Schema Registry 通过 `docs/reference/schema-registry.json` 与 `scripts/generate-forms-from-openapi.js` 等 Plan 300 工具共享；命令/查询服务通过接口层组合 sqlc 生成物，与事务性发件箱、PBAC 守卫共享相同的依赖注入模式。
...
### **7.4 第四阶段：审计与时光机（Effective Dating + Audit/Tx Log）**

1. **事务时间入审计/事件**：主表保持单时态（effective_date/end_date + EXCLUDE），事务时间写入审计表或 Outbox 事件（含 recorded_at/operator/旧值/新值），对齐 Workday 式“有效期主导 + 审计回放”。  
2. **如需强双时态**：在独立历史/版本表增加 `tx_from/tx_to` 或版本号供审计/纠错查询，避免在主表直接堆叠 tx_range 以降低查询复杂度与索引成本。  
3. **时光机 UI**：时间轴既能查看“某时点的生效数据”（有效时间）也能查看“当时系统记录的数据”（基于审计/版本表还原事务时间快照）。
