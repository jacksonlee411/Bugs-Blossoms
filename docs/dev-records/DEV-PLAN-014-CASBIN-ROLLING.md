...
| 验证记录 | dev-records 本文件 | 导航/Quick Links/Spotlight 有/无权账号验证与 403 直访结果尚未登记 | 以有/无能力账号各验证一次可见性与 403 payload，填入本表或日志区 | Done（Core Nav 仅保留 AuthzObject/Action，Forbidden JSON 与 Unauthorized props 三模块统一，go test 覆盖未授权场景） |
