...
| 验证记录 | dev-records 本文件 | 导航/Quick Links/Spotlight 有/无权账号验证与 403 直访结果尚未登记 | 以有/无能力账号各验证一次可见性与 403 payload，填入本表或日志区；e2e 种子已将 NoHRM 账号剥离 `logging.*` 权限，可用于无权侧验证（Playwright logs.spec 覆盖） | In progress（/logs 对超级管理员仍返回 403，logs.spec 首测失败，需确认策略或 AUTHZ 模式） |
